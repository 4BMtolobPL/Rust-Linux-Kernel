...
fn main() -> Result<(), Error> {
    // AtomicBool을 사용해 SIGINT 신호가 수신됐는지 여부를 확인하는 플래그를 설정합니다
    let signal_receive = Arc::new(AtomicBool::new(false));
    // AtomicBool에 대한 참조를 복제합니다. 이것은 signal_hook에 전달됩니다
    let signal_received_clone = signal_receive.clone();
    ...
    while !signal_receive.load(Ordering::Relaxed) {
