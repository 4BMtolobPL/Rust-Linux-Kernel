...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // "com.example.dbustest"라는 이름을 요청해 DBus에 등록합니다
    c.request_name("com.example.dbustest", false, true, false)?;

    // Crossroads 인스턴스를 생성합니다
    let mut cr = Crossroads::new();

    // com.example.dbustest 인터페이스 등록
    let iface_token = cr.register("com.example.dbustest", |b| {
        // "HelloHappend"라는 dbus signal을 설정합니다
        let hello_happend = b.signal("HelloHappend", ("sender",)).msg_fn();
        ...
        b.method(
            ...
            move |ctx: &mut Context, hello: &mut Hello, (name,): (String,)| {
                ...
                let reply = format!("안녕 {name}! API 호출 횟수: {}", hello.called_count);
                let signal_msg = hello_happend(ctx.path(), &(name,));
    ...
    // 생성된 Crossroads 인스턴스를 사용해 DBus 연결에서 메시지를 처리하도록 서비스를 시작합니다
    cr.serve(&c)?;
